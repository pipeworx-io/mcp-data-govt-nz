...
const tools: McpToolExport['tools'] = [
  ...
  { name: 'tag_show', description: 'Single tag.', inputSchema: { type: 'object', properties: { id: { type: 'string' }, include_datasets: { type: 'boolean' } }, required: ['id'] } },
  {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  const get = async (action: string, params?: Record<string, unknown>) => {
    ...
    if (!res.ok) throw new Error(`data.govt.nz: ${res.status}`);
